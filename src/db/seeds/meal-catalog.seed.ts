...
interface SeedProduct {
  ...
  name: string;
  /** Franstalige naam — zelfde rij als `name`: één canoniek record per voedingsmiddel, niet twee. */
  nameFr: string;
  ...
  fat_g: number;
  fiber_g: number;
  /** Mock zoek-/logfrequentie voor rangschikking (databaseplan §3.6 e.v.); geen echte gebruiksdata. */
  popularity: number;
}

/**
 * Generieke Belgische/Nederlandse basisproducten (per 100g, rauw/onbereid tenzij vermeld) in beide
 * landstalen, zodat handmatig zoeken naar alledaagse voeding — in het Nederlands of het Frans —
 * altijd meteen een bruikbaar resultaat geeft, ook zonder barcode en zonder afhankelijk te zijn van
 * of Open Food Facts voor deze exacte zoekterm een goede (volledige) treffer heeft. Richtwaarden
 * gebaseerd op standaard voedingswaardetabellen (NEVO/CIQUAL/USDA).
 */
const GENERIC_PRODUCTS: SeedProduct[] = [
  ...
  { seedKey: 'spek', name: 'Spek', nameFr: 'Lard', calories_kcal: 541, protein_g: 37, carbs_g: 1.4, fat_g: 42, fiber_g: 0, popularity: 20 },
  ...
  { seedKey: 'room-40', name: 'Room 40%', nameFr: 'Crème 40%', calories_kcal: 337, protein_g: 2.2, carbs_g: 3.4, fat_g: 35, fiber_g: 0, popularity: 15 },
...
/**
 * Vult `meal_catalog` met de generieke producten hierboven, of werkt ze bij als ze al bestaan.
 * Eén `bulkCreate`-aanroep met `updateOnDuplicate`/`conflictAttributes` (op `barcode`) in plaats
 * van 39 losse round trips: dit draait in de opstartketen van de service, die op Northflank maar
 * 0,1 vCPU/256 MB krijgt, dus minder round trips en minder tegelijk in-memory werk is hier expliciet
 * de bedoeling, niet alleen sneller.
 */
export async function seedMealCatalog(): Promise<void> {
  const records = GENERIC_PRODUCTS.map((product) => ({
    barcode: `seed:${product.seedKey}`,
    name: product.name,
  ...
  await MealCatalogItem.bulkCreate(records, {
    updateOnDuplicate: [
      'name',
      'name_fr',
      'calories_kcal',
